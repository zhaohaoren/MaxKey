...
async function loadParents() {
  try { parents.value = normalize(await adminGet<unknown>(`${parentBase.value}/fetch`, { pageNumber: 1, pageSize: 1000 })) } catch (err) { message.error(err instanceof Error ? err.message : `${title.value}加载失败`) }
}
...
async function loadMembers() {
  ...
  try {
    ...
    members.value = normalize(await adminGet<unknown>(`${memberBase.value}/${endpoint}`, { [parentKey.value]: selectedParentId.value, username: search.username, pageNumber: 1, pageSize: 1000 }))
  } catch (err) { message.error(err instanceof Error ? err.message : '成员加载失败') } finally { loading.value = false }
}

async function openAdd() {
  if (!selectedParentId.value) { message.warning(`请先选择${kind.value === 'group' ? '用户组' : '角色'}`); return }
  ...
  try {
    ...
    available.value = normalize(await adminGet<unknown>(`${memberBase.value}/${endpoint}`, { [parentKey.value]: selectedParentId.value, pageNumber: 1, pageSize: 1000 }))
...
watch([kind, selectedParentId], loadMembers)
onMounted(loadParents)
</script>

<template>
  <DefaultLayout mode="admin">
    <div class="alain-default__content-title"><div><h1>{{ title }}</h1><small>选择{{ kind === 'group' ? '用户组' : '角色' }}后维护成员</small></div><a-space><a-button @click="loadMembers">刷新</a-button><a-button type="primary" :disabled="!selectedParentId" @click="openAdd">添加成员</a-button></a-space></div>
    <a-row :gutter="16"><a-col :xs="24" :lg="7"><a-card :title="kind === 'group' ? '用户组' : '角色'"><a-input v-model:value="search.parentName" allow-clear placeholder="筛选名称" /><a-list :data-source="parents.filter(item => !search.parentName || String(item.name || item[parentNameKey] || '').includes(search.parentName))" size="small"><template #renderItem="{ item }"><a-list-item class="assignment-parent" :class="{ selected: String(item.id) === selectedParentId }" @click="selectedParentId = String(item.id)">{{ item.name || item[parentNameKey] }}</a-list-item></template></a-list></a-card></a-col>
      <a-col :xs="24" :lg="17"><a-card :title="selectedParent ? `${selectedParent.name || selectedParent[parentNameKey]} 的成员` : '成员列表'"><a-form layout="inline" @submit.prevent="loadMembers"><a-form-item label="用户名"><a-input v-model:value="search.username" allow-clear /></a-form-item><a-button type="primary" html-type="submit" :disabled="!selectedParentId">查询</a-button></a-form><a-table :data-source="members" :loading="loading" row-key="id" :pagination="false"><a-table-column title="用户名" data-index="username" /><a-table-column title="显示名称"><template #default="{ record }">{{ record.displayName || record.memberName }}</template></a-table-column><a-table-column title="类型" data-index="type" /><a-table-column title="操作"><template #default="{ record }"><a-button danger type="link" @click="removeMember(record)">移除</a-button></template></a-table-column></a-table></a-card></a-col></a-row>
